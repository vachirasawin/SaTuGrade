...
const AL_MODEL_API_URL = process.env.AL_MODEL_API_URL;

export async function POST(request) {
    try {
        if (!AL_MODEL_API_URL) {
            return NextResponse.json(
                { success: false, error: "ยังไม่ได้ตั้งค่า AL_MODEL_API_URL ใน .env" },
                { status: 500 }
            );
        }
        ...
        const response = await fetch(`${AL_MODEL_API_URL}/predict`, {
            method: "POST",
        ...
        console.error("Error in /api/models/AL:", error);
        return NextResponse.json(
